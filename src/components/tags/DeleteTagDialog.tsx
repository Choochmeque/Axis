...
export function DeleteTagDialog({ isOpen, onClose, tag, remotes }: DeleteTagDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      {isOpen && <DeleteTagDialogContent onClose={onClose} tag={tag} remotes={remotes} />}
    </Dialog>
  );
}

interface DeleteTagDialogContentProps {
  onClose: () => void;
  tag: Tag;
  remotes: Remote[];
}

function DeleteTagDialogContent({ onClose, tag, remotes }: DeleteTagDialogContentProps) {
