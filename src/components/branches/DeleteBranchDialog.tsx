...
export function DeleteBranchDialog({ isOpen, onClose, branch }: DeleteBranchDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      {isOpen && <DeleteBranchDialogContent onClose={onClose} branch={branch} />}
    </Dialog>
  );
}

interface DeleteBranchDialogContentProps {
  onClose: () => void;
  branch: Branch;
}

function DeleteBranchDialogContent({ onClose, branch }: DeleteBranchDialogContentProps) {
    ...
    <DialogContent>
      ...
      <DialogBody>
        ...
        <div className="dialog-info-box">
          ...
          <div className="flex justify-between text-base py-1">
            <span className="text-(--text-secondary)">{t('branches.delete.lastCommitLabel')}</span>
