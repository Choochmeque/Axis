...
describe('Unstage Workflow', () => {
  let tempDir: string;

  before(async () => {
    // Create a git repo with an initial commit and a tracked file
    tempDir = createTempGitRepo({
      files: { 'unstage-test.txt': 'Initial content' },
      initialCommit: true,
    });

    // Modify the tracked file so it shows as unstaged
    modifyFile(tempDir, 'unstage-test.txt', 'Modified content for unstage test');
  ...
  it('should unstage the file by clicking checkbox again', async () => {
    // Click the checkbox in the staged section to unstage
    const checkbox = await $(SELECTORS.STAGING_FILE_CHECKBOX);
    await checkbox.waitForExist({ timeout: 5_000 });
    await checkbox.click();
    ...
    // Verify file is back in unstaged - unstaged header should exist
