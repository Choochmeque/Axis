...
function ArchiveDialogContent({ onClose, commitOid, commitSummary }: ArchiveDialogContentProps) {
  const { t } = useTranslation();
  const repository = useRepositoryStore((state) => state.repository);
  const [format, setFormat] = useState<ArchiveFormat>('zip');
  const [outputPath, setOutputPath] = useState('');
  const [prefix, setPrefix] = useState(repository?.name ? `${repository.name}/` : '');
  ...
  return (
    <DialogContent className="max-w-112.5">
      ...
      <DialogFooter>
        ...
        <Button variant="primary" onClick={handleCreate} disabled={isLoading || !outputPath.trim()}>
