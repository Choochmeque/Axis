...
export namespace GG {
	export enum GraphStyle { Rounded = 0, Angular = 1 }
	export enum GraphUncommittedChangesStyle { OpenCircleAtTheCheckedOutCommit = 0, OpenCircleAtTheUncommittedChanges = 1 }
	export interface GraphConfig {
		colours: string[];
		style: GraphStyle;
		grid: { x: number; y: number; offsetX: number; offsetY: number; expandY: number };
		uncommittedChanges: GraphUncommittedChangesStyle;
	}
...
// Stub functions - replace with React equivalents or remove
function findCommitElemWithId(elems: HTMLCollectionOf<HTMLElement>, id: number): HTMLElement | null {
	for (let i = 0; i < elems.length; i++) {
		if (parseInt(elems[i].dataset.id || '-1') === id) return elems[i];
	}
	return null;
}
...
function closeDialogAndContextMenu() { /* noop */ }
...
function getBranchLabels(heads: string[], remotes: GG.GitCommitRemote[]): { heads: { name: string; remotes: string[] }[]; remotes: GG.GitCommitRemote[] } {
	return {
...
export class Graph {
	...
	constructor(elem: HTMLElement, viewElem: HTMLElement, config: GG.GraphConfig, muteConfig: GG.MuteCommitsConfig) {
		this.viewElem = viewElem;
		this.config = config;
		this.muteConfig = muteConfig;

		this.contentElem = elem.parentElement ?? elem;
