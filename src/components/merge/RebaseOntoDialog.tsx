...
import { type Branch, BranchType, type RebaseResult } from '@/types';
...
interface RebaseOntoDialogContentProps {
  onClose: () => void;
  onRebaseComplete?: (result: RebaseResult) => void;
  currentBranch: string;
  newBase: string;
...
function RebaseOntoDialogContent({
  onClose,
  onRebaseComplete,
  currentBranch,
  newBase,
}: RebaseOntoDialogContentProps) {
  ...
  const {
    ...
    queryKey: ['rebase-onto-dialog-data'],
    queryFn: async () => {
      ...
      return { allBranches, recentCommits };
    },
  });

  const branches: Branch[] = (data?.allBranches ?? []).filter(
    (b: Branch) => b.name !== currentBranch && b.name !== newBase && !b.isHead
  );
  const commits = data?.recentCommits ?? [];
  const effectiveError = error ?? (dataError ? getErrorMessage(dataError) : null);
          ...
          <>
            ...
            {isLoadingData ? (
              <div className="flex items-center justify-center p-4">
                ...
                <span className="ml-2 text-sm text-(--text-secondary)">{t('common.loading')}</span>
              </div>
            ) : (
              <FormField
